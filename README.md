Journeys

A SwiftUI app for tracking train journeys, collecting commuter pass stamps, and building travel streaks. Create custom rail operators, start timed journeys, and watch your stats grow.

Requirements

iOS 17.0+
Xcode 15+
Swift 5.9+

Setup

Clone the repository
Open in Xcode 15 or later
Build and run on an iOS 17 simulator or device

License

Copyright (c) 2026 sam
You are permitted to view and build and modify this source code for personal, non-commercial use only.
You may not distribute, sublicense, or create derivative works from this code. You may not use this code, in whole or in part, for any commercial purpose or for profit. All rights not expressly granted are reserved by the author.
This software is provided "as is", without warranty of any kind, express or implied.
